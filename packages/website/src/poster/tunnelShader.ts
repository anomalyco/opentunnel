...
void main() {
  ...
  // Vanishing point sits high; the sphere rests below it.
  vec2 eye = vec2(0.0, 0.3);
  vec2 d = uv - eye;
  float r = length(d);
  ...
  float warp = fbm(ring * 2.4 + vec2(0.0, z * 0.45 - t * 1.5)) * 2.2;
  float bands = 0.5 + 0.5 * sin(z * 4.2 - t * 6.0 + warp);
  ...
  float shore = smoothstep(-0.12, -0.55, uv.y);
  sea = (0.25 + 0.75 * smoothstep(0.35, 0.7, sea)) * shore + 0.3 * smoothstep(-0.6, -0.85, uv.y);
